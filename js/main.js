...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Testimonials Slider (Generic function for both member and client testimonials) ---
  function initTestimonialSlider(slidesContainerId, dotsContainerId) {
    ...
    // Auto-slide every 6 seconds (only if user doesn't prefer reduced motion)
    if (!prefersReducedMotion) {
      autoSlideInterval = setInterval(nextSlide, 6000);
    }
  }
  
  // Initialize member testimonials (on index page)
  initTestimonialSlider('testimonialSlides', 'testimonialDots');
  
  // Initialize client testimonials (on projects page)
  initTestimonialSlider('clientTestimonialSlides', 'clientTestimonialDots');
});
